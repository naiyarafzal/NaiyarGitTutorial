...
df_src=spark.sql('''
            SELECT distinct(Branch_ID) as Branch_ID, BranchName FROM 
...
if spark.catalog.tableExists('cars_catalog.gold.dim_branch'):
    delta_tbl=DeltaTable.forPath(spark,'abfss://gold@carstorageadls.dfs.core.windows.net/dim_branch')
    ...
    df_final.write.format('delta')\
        .mode('overwrite')\
            .option('path','abfss://gold@carstorageadls.dfs.core.windows.net/dim_branch')\
